...
ladies_x <- read_excel("/Users/syverjohansen/ski/elo/python/ski/relay/radar/men_chrono_regress.xlsx", 
                       sheet = "Sheet1", col_names = TRUE, na = "NA", guess_max = 10000)
...
df_cols = names(ladies[c(15, 17, 19, 21, 23, 25, 27, 29, 31)])
df_cols
...
#full.model <- lm((points)^(1/exp(1)) ~ ., data=ladies[,c(15, 17, 19, 21, 23, 25, 27, 29, 31, 53)])
...
ladies.lm <- lm((points)^(1/exp(1)) ~ distance_pelo + distance_freestyle_pelo + distance_classic_pelo + sprint_pelo + sprint_freestyle_pelo  
                  , data=ladies,  na.action = na.omit, family="gaussian")
...
ladies.lm <- lm((points)^(1/exp(1)) ~ distance_pelo, data=ladies,  na.action = na.omit, family="gaussian")
AIC(ladies.lm)
...
step <- stepAIC(ladies.lm, direction="both")
step$anova
