...
def get_table(olympic_page):
	try:
		...
		table =  ["", "", "", "", ""]
		return table
	body = olympic_soup.find_all("tr")
	body = body[-5:-1]
	td = olympic_soup.find_all('td')
	body = td[-10:-1]
	date = body[-2].text
	date = str(date)
	date = date.split(' ')
	...
	if(int(year)>1985 and distance!="Rel"):
		...
		return table
	#return olympic_date
def get_skier(olympic_page, distance, sex2):
	try:
		olympic_soup = BeautifulSoup(urlopen(olympic_page), 'html.parser')
	except:
		places = ""
...
def get_olympic():
	...
	olympics_years = [1960,1964, 1968, 1972, 1976, 1980, 1984, 1988, 1992, 1994, 1998, 2002, 2006, 2010, 2014, 2018]
	for a in range(len(olympics_years)):
		men_olympic_page0 = "https://skisport365.com/skiskyting/rennkalender.php?aar="+str(olympics_years[a])+"&hva=OLVM"
		ladies_olympic_page0 = "https://skisport365.com/skiskyting/rennkalender.php?aar="+str(olympics_years[a])+"&hva=OLVM&k=F"
		...
		for a in range(2, len(ladiesbody), 8):
			...
			if(ladiesbody[a].text==""):
				...
				num-=1
			#	ladies_olympic_page1.append('https://skisport365.com/skiskyting/'+ladiesbody[a+2]['href'])
			num+=1
		#print(ladies_wcevents)
		it = 0
		for b in men_olympic_soup0.find_all('a', {'class':'ablue'}, href = True):
			if(it in men_wcevents):
				men_olympic_page1.append('https://skisport365.com/skiskyting/'+b['href'])
			it+=1
		...
		for b in ladies_olympic_soup0.find_all('a', {'class':'ablue'}, href=True):
			...
			if(it in ladies_wcevents):
				ladies_olympic_page1.append('https://skisport365.com/skiskyting/'+b['href'])
			it+=1
	...
	for a in range(len(men_olympic_page1)):
		...
		skiers = get_skier(men_olympic_page1[a], distance, sex)
	...
	for a in range(len(ladies_olympic_page1)):
		...
		skiers = get_skier(ladies_olympic_page1[a], distance, sex)
		places = skiers[0]
		ski = skiers[1]
		nation = skiers[2]
...
workbook = xlsxwriter.Workbook("/Users/syverjohansen/ski/excel365/biathlon/olympics/olympic.xlsx")
	...
	if(g==0):
		...
		for a in range(len(olympic[g])):
			for b in range(len(olympic[g][a][7])):
				...
				men.write(row, col+4, olympic[g][a][4][b])
