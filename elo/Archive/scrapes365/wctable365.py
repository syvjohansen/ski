...
def get_standings():
	mentable = []
	ladiestable = []
	for a in range(1982, 2020):

		print(a)
		year = int(str(a)+"9999")
		men_table_page0 = "https://skisport365.com/ski/ranking.php?aar="+str(a)
		ladies_table_page0 = "https://skisport365.com/ski/ranking.php?aar="+str(a)+"&hva=&k=F"
		...
		if( a<=2006):
			...
			people = 1		
			for a in range(0, len(menbody), 14):
				people+=1
			...
			people = 1
			for a in range(0, len(ladiesbody), 14):
				people+=1
				ladiesplaces.append(ladiesbody[a+3].text)
				ladiesskier.append(ladiesbody[a].text.strip('\n'))
				ladiesnation.append(ladiesbody[a+2].text.strip('\n'))
...
workbook = xlsxwriter.Workbook("/Users/syverjohansen/ski/excel365/table/wctable.xlsx")
