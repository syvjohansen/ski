...
def knapsack(data):
  ...
  men, women = [], []
  for j in range(data.shape[0]):
    if data["gender"].iloc[j] == "m":
      men.append(x[j])
    elif data["gender"].iloc[j] == "f":
      women.append(x[j])
    else:
  ...
  for j in range(L):
    objective.SetCoefficient(x[j], float(data['score'].iloc[j]))
  ...
  if status == pywraplp.Solver.OPTIMAL:
    ...
    ## Print Picks:
    print(data[data.Solution > 0].sort_values(by=['gender', 'price'],ascending=False))
  else:
